...
def pred_with_shuffled_cov(cov, w_path):        
    def shuffle_cov(X, cov):
        '''Reorder the pixels in the extraction window'''
        X_cov = X[:, :, :, cov]
        rng = np.random.default_rng()
        rng.shuffle(X_cov, axis=1)
        rng.shuffle(X_cov, axis=2)
...
for i in range(0, 103):
    ...
    np.save('./mu_local.npy', arr_0)
    np.save('./sigma_local.npy', arr_1)
    print('FP_%i Done ...' %i)
...
# df_output.to_csv('./fp_percentile_change_local.csv')